...
for img_source in ['highway', 'office', 'pedestrants']:
    ...
    Buffer = np.zeros(shape=(YY, XX, buffer_size), dtype=np.uint8)
    I_model = np.zeros(shape=(YY, XX), dtype=np.uint8)

    for n, img in enumerate(range(roi_start-buffer_size, roi_start, step_frame)):
        I = cv.imread(img_source + '/input/in%06d.jpg' % img)
    ...
    for i in range(YY):
        for j in range(XX):
            counts = np.bincount(Buffer[i, j, :])
            I_model[i, j] = np.argmax(counts)
    ...
    for i in range(roi_start, roi_end, step_frame):
        ...
        cv.imshow('Video: median method', video)
